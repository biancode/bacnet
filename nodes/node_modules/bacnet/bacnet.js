...
function BACnetInstance (config) {
  ...
  // bacnetAddon.initDevice(this)
  bacnetAddon.listen(this)
...
function setupMethods (bacnetAddon, confirmedCallbacks) {
  ...
  this.whoHasName = bacnetAddon.whoHasName
  this.whoHasObject = bacnetAddon.whoHasObject
  this.isBound = bacnetAddon.isBound
  this.initDevice = bacnetAddon.initDevice
  this.listen = bacnetAddon.listen
  ...
  this.deviceCommControl = function (deviceAddress, deviceInstance, timeDuration, state, password, callback) {
    const invokeId = bacnetAddon.deviceCommControl(deviceAddress, deviceInstance, timeDuration, state, password)
    if (invokeId === 0) throw new Error('Invoking BACnet Device Comm service failed!')
    return addCallback(invokeId, callback)
  }
  this.reInitDevice = function (deviceAddress, deviceInstance, state, password, callback) {
    const invokeId = bacnetAddon.reInitDevice(deviceAddress, deviceInstance, state, password)
    if (invokeId === 0) throw new Error('Invoking BACnet Re-Initialize Device failed!')
    return addCallback(invokeId, callback)
  }
  this.subscribeCov = function (dest, subscriberProcessIdentifier, monitoredObjectIdType, monitoredObjectIdInstance, issueConfirmedNotifications, lifetime, covIncrement, callback) {
    const invokeId = bacnetAddon.subscribeCov(dest, subscriberProcessIdentifier, monitoredObjectIdType, monitoredObjectIdInstance, issueConfirmedNotifications, lifetime, covIncrement, callback)
    if (invokeId === 0) throw new Error('Invoking BACnet S-Cov failed!')
    return addCallback(invokeId, callback)
  }
  // this.readPropertyMultiple = function (dest, callback) {
  //   const invokeId = bacnetAddon.readPropertyMultiple(dest, callback)
  //   if (invokeId === 0) throw new Error('Invoking BACnet RPM failed!')
  //   return addCallback(invokeId, callback)
  // }
  // this.whoHasName = function (lowLimit, highLimit, objectName, callback) {
  //   const invokeId = bacnetAddon.whoHasName(lowLimit, highLimit, objectName)
  //   if (invokeId === 0) throw new Error('Invoking BACnet whoHasName failed!')
  //   return addCallback(invokeId, callback)
  // }
  // this.whoHasObject = function (lowLimit, highLimit, objectType, objectInstance, callback) {
  //   const invokeId = bacnetAddon.whoHasObject(lowLimit, highLimit, objectType, objectInstance)
  //   if (invokeId === 0) throw new Error('Invoking BACnet whoHasObject failed!')
  //   return addCallback(invokeId, callback)
  // }
}
