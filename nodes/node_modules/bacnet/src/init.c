...
void init_service_handlers() {
    ...
    apdu_set_error_handler(SERVICE_UNCONFIRMED_I_AM, handler_error);

    apdu_set_unconfirmed_handler(SERVICE_UNCONFIRMED_I_HAVE, handler_i_have);
    apdu_set_error_handler(SERVICE_UNCONFIRMED_I_HAVE, handler_error);
...
void init_device_service_handlers() {
    ...
    apdu_set_error_handler(SERVICE_UNCONFIRMED_WHO_IS, handler_error);

    // adding user specific handlers
    // Akhil <amkurup@mtu.edu>
    // 21/02/2017
    apdu_set_unconfirmed_handler(SERVICE_UNCONFIRMED_WHO_HAS, handler_who_has);
    apdu_set_error_handler(SERVICE_UNCONFIRMED_WHO_HAS, handler_error);

    apdu_set_confirmed_handler(SERVICE_CONFIRMED_READ_PROPERTY, handler_read_property);
    apdu_set_error_handler(SERVICE_CONFIRMED_READ_PROPERTY, handler_error);

    apdu_set_confirmed_handler(SERVICE_CONFIRMED_READ_PROP_MULTIPLE, handler_read_property_multiple);
    apdu_set_error_handler(SERVICE_CONFIRMED_READ_PROP_MULTIPLE, handler_error);

    apdu_set_confirmed_handler(SERVICE_CONFIRMED_WRITE_PROPERTY, handler_write_property);
    apdu_set_error_handler(SERVICE_CONFIRMED_WRITE_PROPERTY, handler_error);

    apdu_set_confirmed_handler(SERVICE_CONFIRMED_WRITE_PROP_MULTIPLE, handler_write_property_multiple);
    apdu_set_error_handler(SERVICE_CONFIRMED_WRITE_PROP_MULTIPLE, handler_error);

    apdu_set_unconfirmed_handler(SERVICE_UNCONFIRMED_UTC_TIME_SYNCHRONIZATION,
        handler_timesync_utc);
    apdu_set_error_handler(SERVICE_UNCONFIRMED_UTC_TIME_SYNCHRONIZATION,
        handler_error);

    apdu_set_unconfirmed_handler(SERVICE_UNCONFIRMED_TIME_SYNCHRONIZATION, handler_timesync);
    apdu_set_error_handler(SERVICE_UNCONFIRMED_TIME_SYNCHRONIZATION, handler_error);

    apdu_set_confirmed_handler(SERVICE_CONFIRMED_SUBSCRIBE_COV, handler_cov_subscribe);
    apdu_set_error_handler(SERVICE_CONFIRMED_SUBSCRIBE_COV, handler_error);

    apdu_set_unconfirmed_handler(SERVICE_UNCONFIRMED_COV_NOTIFICATION, handler_ucov_notification);
    apdu_set_error_handler(SERVICE_UNCONFIRMED_COV_NOTIFICATION, handler_error);
    ...
    apdu_set_error_handler(SERVICE_CONFIRMED_REINITIALIZE_DEVICE, handler_error);
}
