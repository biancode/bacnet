// for addon
#include <node.h>
#include <v8.h>
#include <nan.h>
#include <string>
#include <iostream>
#include <sstream>
#include <typeinfo>
#include "functions.h"
#include "listen.h"
#include "init.h"
#include "listenable.h"
#include "client.h"
#include "newclient.h"
#include "bactext.h"
#include "address.h"
#include "conversion.h"
#include "BacnetValue.h"


NAN_METHOD(objectTypeToString) {
    std::ostringstream errorStringStream;
    std::string inputString = extractString(info[0].As<v8::String>());
    if (info.Length() >= 1 && info[0]->IsString()) {
        unsigned index;
        if (bactext_object_type_index(extractString(info[0].As<v8::String>()).c_str(), &index)) {
            const char * name = bactext_object_type_name(index);
            info.GetReturnValue().Set(Nan::New(name).ToLocalChecked());
        } else {
            errorStringStream << "Object type string not valid" << ", provided : " << inputString;
            Nan::ThrowError(errorStringStream.str().c_str());
        }
    } else if (info.Length() >= 1 && info[0]->IsUint32()) {
        const char * name = bactext_object_type_name(info[0]->ToUint32()->Value());
        info.GetReturnValue().Set(Nan::New(name).ToLocalChecked());
    } else {
        errorStringStream << "Object type must be either a string or unsigned int" << ", provided : " << inputString;
        Nan::ThrowError(errorStringStream.str().c_str());
    }
}
NAN_METHOD(objectTypeToNumber) {
    std::ostringstream errorStringStream;
    std::string inputString = extractString(info[0].As<v8::String>());
    if (info.Length() >= 1) {
        unsigned value;
        const char * error = objectTypeToC(info[0], &value);
        if (error) {
            errorStringStream << "Object type string not valid : " << error << ", provided : " << inputString;
            Nan::ThrowError(errorStringStream.str().c_str());
        } else {
            info.GetReturnValue().Set(Nan::New(value));
        }
    } else {
        errorStringStream << "Object type must be either a string or unsigned int" << ", provided : " << inputString;
        Nan::ThrowError(errorStringStream.str().c_str());
    }
}
NAN_METHOD(propertyKeyToString) {
    std::ostringstream errorStringStream;
    std::string inputString = extractString(info[0].As<v8::String>());
    if (info.Length() >= 1 && info[0]->IsString()) {
        unsigned index;
        if (bactext_property_index(inputString.c_str(), &index)) {
            const char * name = bactext_property_name(index);
            info.GetReturnValue().Set(Nan::New(name).ToLocalChecked());
        } else {
            errorStringStream << "Property key string not valid" << ", provided : " << inputString;
            Nan::ThrowError(errorStringStream.str().c_str());
        }
    } else if (info.Length() >= 1 && info[0]->IsUint32()) {
        uint32_t propertyKey = info[0]->ToUint32()->Value();
        if (propertyKey <= MAX_BACNET_PROPERTY_ID) {
            const char * name = bactext_property_name(propertyKey);
            info.GetReturnValue().Set(Nan::New(name).ToLocalChecked());
        } else {
            errorStringStream << "Property key too large, maximum is 4194303" << ", provided : " << inputString;
            Nan::ThrowRangeError(errorStringStream.str().c_str());
        }
    } else {
        errorStringStream << "Property key must be either a string or unsigned int" << ", provided : " << inputString;
        Nan::ThrowError(errorStringStream.str().c_str());
    }
}
NAN_METHOD(propertyKeyToNumber) {
    std::ostringstream errorStringStream;
    std::string inputString = extractString(info[0].As<v8::String>());
    if (info.Length() >= 1 && info[0]->IsString()) {
        unsigned index;
        if (bactext_property_index(inputString.c_str(), &index)) {
            info.GetReturnValue().Set(Nan::New(index));
        } else {
            errorStringStream << "Property key string not valid" << ", provided : " << inputString;
            Nan::ThrowError(errorStringStream.str().c_str());
        }
    } else if (info.Length() >= 1 && info[0]->IsUint32()) {
        uint32_t propertyKey = info[0]->ToUint32()->Value();
        if (propertyKey <= MAX_BACNET_PROPERTY_ID) {
            info.GetReturnValue().Set(Nan::New(propertyKey));
        } else {
            errorStringStream << "Property key too large, maximum is 4194303" << ", provided : " << inputString;
            Nan::ThrowRangeError(errorStringStream.str().c_str());
        }
    } else {
        errorStringStream << "Property key must be either a string or unsigned int" << ", provided : " << inputString;
        Nan::ThrowError(errorStringStream.str().c_str());
    }
}

// whois([destination, [min_id , [max_id]]])
NAN_METHOD(whois) {
    int32_t min_id = -1;
    int32_t max_id = -1;
    BACNET_ADDRESS dest = bacnetAddressToC(info[0]);

    if (info.Length() > 1) {
        min_id = max_id = info[1]->ToInt32()->Value();
    }
    if (info.Length() > 2) {
        max_id = info[2]->ToInt32()->Value();
    }

    if (min_id > BACNET_MAX_INSTANCE) {
        fprintf(stderr, "device-instance-min=%u - it must be less than %u\n",
            min_id, BACNET_MAX_INSTANCE + 1);
        return;
    }
    if (max_id > BACNET_MAX_INSTANCE) {
        fprintf(stderr, "device-instance-max=%u - it must be less than %u\n",
            max_id, BACNET_MAX_INSTANCE + 1);
        return;
    }
    /* send the request */
    Send_WhoIs_To_Network(&dest, min_id, max_id);

    info.GetReturnValue().Set(Nan::New(true));
}

bool addressOrBoundDeviceIdToC(Local<Value> value, unsigned * max_apdu, BACNET_ADDRESS * dest) {
    if (value->IsNumber()) {  // device id
        int32_t device_id = value->ToInt32()->Value();

        /* is the device bound? */
        bool isBound = address_get_by_device(device_id, max_apdu, dest);
        if (!isBound) {
            Nan::ThrowError("device is not bound\n");
            return false;
        }
    } else {   // device address
        *dest = bacnetAddressToC(value);
        *max_apdu = MAX_APDU; // without doing the whois we dont know the Max apdu for the device - so we will just hope it is the same as ours
    }
    return true;
}

// reInitDevice(device_id, state, password)
NAN_METHOD(reInitDevice) {
    BACNET_ADDRESS dest = {};
    unsigned max_apdu = 0;

    int32_t device_id = info[1]->ToInt32()->Value();
    int32_t state = info[2]->ToInt32()->Value();
    char *password = NULL;

    // password = info[4];
    password = "Jesus";

    bool addressed = addressOrBoundDeviceIdToC(info[0], &max_apdu, &dest);

    if (addressed) {
        // invoke service
        int invoke_id = Send_Reinitialize_Device_Request_New(
                            &dest,
                            device_id,
                            (BACNET_REINITIALIZED_STATE)state,
                            password);

        info.GetReturnValue().Set(Nan::New(invoke_id));
    } else {
        Nan::ThrowError("Unable to resolve address for reInitDevice!");
    }
}

// timeSync()
NAN_METHOD(timeSync) {
  // invoke the TimeSync function
  Send_TimeSync_Device();
  // return
  info.GetReturnValue().Set(Nan::New(true));
}

// dcc(device_id, timeDuration, state, password)
// State: 0-> Enable; 1-> Disable; 2-> Disable Comm. Init.;
NAN_METHOD(deviceCommControl) {

    BACNET_ADDRESS dest = {};
    unsigned max_apdu = 0;

    int32_t device_id = info[1]->ToInt32()->Value();
    int32_t time_duration = info[2]->ToInt32()->Value();
    int32_t state = info[3]->ToInt32()->Value();
    char *password = NULL;

    password = "filister";

    bool addressed = addressOrBoundDeviceIdToC(info[0], &max_apdu, &dest);

    if (addressed) {
        // invoke service
        int invoke_id = Send_Device_Communication_Control_Request_New(
                            &dest,
                            device_id,
                            time_duration,
                            (BACNET_COMMUNICATION_ENABLE_DISABLE)state,
                            password);

        // return ID
        info.GetReturnValue().Set(Nan::New(invoke_id));
    } else {
        Nan::ThrowError("Unable to resolve address for deviceCommControl!");
    }
}

// subscribeCov(addr, subProcessId, type, instance, [issueNotification, lifetime])
NAN_METHOD(subscribeCov) {
    BACNET_ADDRESS dest = {};
    unsigned max_apdu = 0;
    bool addressed = addressOrBoundDeviceIdToC(info[0], &max_apdu, &dest);

    uint32_t subscriberProcessIdentifier = info[1]->ToUint32()->Value();
    // BACNET_OBJECT_ID monitoredObjectId
    uint32_t monitoredObjectIdType = info[2]->ToUint32()->Value(); // uint16_t
    uint32_t monitoredObjectIdInstance = info[3]->ToUint32()->Value(); // uint32_t
    // BACNET_PROPERTY_REFERENCE monitoredProperty
    //
    bool issueConfirmedNotifications = info[4]->ToBoolean()->Value(); // bool (optional)
    uint32_t lifetime = info[5]->ToUint32()->Value(); // uint32_t (optional)
    bool covIncrementPresent = info[6]->ToBoolean()->Value(); // bool
    uint32_t covIncrement = info[7]->ToUint32()->Value(); // uint32_t (optional)

    if (addressed) {
        // invoke service
        int invoke_id = Send_COV_Subscribe_New(
                            &dest,
                            subscriberProcessIdentifier,
                            monitoredObjectIdType,
                            monitoredObjectIdInstance,
                            issueConfirmedNotifications,
                            lifetime,
                            covIncrement);
        // return ID
        info.GetReturnValue().Set(Nan::New(invoke_id));
    } else {
        Nan::ThrowError("Unable to resolve address for subscribeCov!");
    }
}

// whoHasName(low_limit, high_limit, object_name)
NAN_METHOD(whoHasName) {

    int32_t low_limit = info[0]->ToInt32()->Value();
    int32_t high_limit = info[1]->ToInt32()->Value();
    char *object_name = NULL;

    object_name = "analog-value";

    Send_WhoHas_Name(
        low_limit,
        high_limit,
        object_name);
}

// whoHasObject(low_limit, high_limit, object_type, object_instance)
NAN_METHOD(whoHasObject) {

    int32_t low_limit = info[1]->ToInt32()->Value();
    int32_t high_limit = info[2]->ToInt32()->Value();
    int32_t object_type = info[3]->ToInt32()->Value();
    int32_t object_instance = info[4]->ToInt32()->Value();

    Send_WhoHas_Object(
        low_limit,
        high_limit,
        (BACNET_OBJECT_TYPE)object_type,
        object_instance);

    info.GetReturnValue().Set(Nan::New(true));
}

// // readPropertyMultiple(addr, subProcessId, type, instance, [issueNotification, lifetime])
// NAN_METHOD(readPropertyMultiple) {

//     BACNET_ADDRESS dest = {};
//     unsigned max_apdu = 0;
//     bool addressed = addressOrBoundDeviceIdToC(info[0], &max_apdu, &dest);

//     if (addressed) {
//         int invoke_id = Send_Read_Property_Multiple_Request_New(
//                         &dest,
//                             );

//         info.GetReturnValue().Set(Nan::New(invoke_id));
//     } else {
//         Nan::ThrowError("Unable to resolve address for read.");
//     }
// }

// readProperty(deviceId, objectType, objectId, property [, arrayIndex])
NAN_METHOD(readProperty) {

    BACNET_ADDRESS dest = {};
    unsigned max_apdu = 0;

    bool addressed = addressOrBoundDeviceIdToC(info[0], &max_apdu, &dest);
    int32_t object_type = info[1]->ToInt32()->Value();
    int32_t object_instance = info[2]->ToInt32()->Value();
    int32_t object_property = info[3]->ToInt32()->Value();
    uint32_t array_index = BACNET_ARRAY_ALL;

    if (info[4]->IsUint32()) {
        array_index = info[4]->ToUint32()->Value();
    }

    if (addressed) {
        int invoke_id = Send_Read_Property_Request_Address(
            &dest,
            max_apdu,
            (BACNET_OBJECT_TYPE)object_type,
            object_instance,
            (BACNET_PROPERTY_ID)object_property,
            array_index);

        info.GetReturnValue().Set(Nan::New(invoke_id));
    } else {
        Nan::ThrowError("Unable to resolve address for read.");
    }
}

// isBound(deviceId) - checks whether a deviceId is already bound and can be used to address a device
NAN_METHOD(isBound) {
    BACNET_ADDRESS dest = {};
    unsigned max_apdu = 0;

    int32_t device_id = info[0]->ToInt32()->Value();
    bool bound = address_get_by_device(device_id, &max_apdu, &dest);
    info.GetReturnValue().Set(Nan::New(bound));
}

// writeProperty(deviceId, objectType, objectId, property, arrayIndex, value [, priority])
NAN_METHOD(writeProperty) {
    BACNET_ADDRESS dest = {};
    unsigned max_apdu = 0;

    bool addressed = addressOrBoundDeviceIdToC(info[0], &max_apdu, &dest);
    int32_t object_type = info[1]->ToInt32()->Value();
    int32_t object_instance = info[2]->ToInt32()->Value();
    int32_t object_property = info[3]->ToInt32()->Value();
    uint32_t array_index = BACNET_ARRAY_ALL;
    if (info[4]->IsUint32()) {
        array_index = info[4]->ToUint32()->Value();
    }

    Local<Object> valueObject = Nan::To<Object>(info[5]).ToLocalChecked();

    uint8_t priority = BACNET_NO_PRIORITY;
    if (info[6]->IsUint32()) {
        priority = info[6]->ToUint32()->Value();
    }

    if (addressed) {
        BacnetValue * bacnetValue = BacnetValue::Unwrap<BacnetValue>(valueObject);
        BACNET_APPLICATION_DATA_VALUE object_value = {};
        if (bacnetValue->bacnetValue(&object_value)) {
            int invoke_id = Send_Write_Property_Request_Address(
                &dest,
                max_apdu,
                (BACNET_OBJECT_TYPE)object_type,
                object_instance,
                (BACNET_PROPERTY_ID)object_property,
                &object_value,
                priority,
                array_index);
            info.GetReturnValue().Set(Nan::New(invoke_id));
        } else {
            Nan::ThrowError("Some kind of error converting bacnet value for write property");
        }
    } else {
        Nan::ThrowError("Unable to resolve address for write.");
    }
}

NAN_METHOD(listen) {
  listenLoop();
}

NAN_METHOD(initClient) {
  v8::Local<v8::Object> localEventEmitter = info[0]->ToObject();
  eventEmitterSet(localEventEmitter);

  init_service_handlers();
}

NAN_METHOD(initDevice) {
  init_device_service_handlers();
}

NAN_METHOD(closeQueue) {
  eventEmitterClose();
}
